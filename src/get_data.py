...
class GetUpdateData:
    ...
    def get_tickers(self):
        ...
        if self.update_existing:
            df_tickers = self.existing_data[['ticker', 'Security', 'GICS Sector', 'GICS Sub-Industry', 'Headquarters Location', 'Date added', 'CIK', 'Founded']]
            ...
            df_tickers['ticker'] = np.where(df_tickers['ticker'].isin(['GOOG', 'GOOGL']), 'GOOGL', df_tickers['ticker'])
            df_tickers.drop_duplicates(subset='ticker', inplace=True)
            
            try:
                df_tickers = df_tickers[~df_tickers['ticker'].isin(self.existing_data['ticker'])]
            except:
                pass
            
            limit = min(200, len(df_tickers))
            tickers = list(df_tickers.iloc[0:limit, df_tickers.columns.get_loc('ticker')])
    ...
    def get_data_updates(self):
        ...
        for ticker in tickers:
            ...
            try:
                start_date = (pd.to_datetime(self.existing_data[self.existing_data['ticker']==ticker]['datetime']) + timedelta(days=1)).dt.strftime('%Y-%m-%d').values[0]
                end_date = date.today().strftime('%Y-%m-%d')
