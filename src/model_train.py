...
class PrepData(self):
    ...
    """
    def __init__(self, df, date_col, ticker_col, close_col, targ_direction=False, targ_perc_return=False):
        self.df = df
    ...
    def generate_target(self):
        self.df['7dClose'] = self.df.groupby(self.ticker_col)[self.date_col].apply(self.find_value_7_days_later)
        if self.targ_direction:
            self.df['target'] = np.where(self.df[self.close_col] > self.df['7dClose'], 'up', 'down')
            
        elif self.targ_perc_return:
            self.df['target'] = (self.df['7dClose'] - self.df[self.close_col]) / self.df['7dClose']
        
        else:
            raise ValueError("Either Targ_direction or Targ_perc_return has to be True")
...
class CreateModel(self):
    ...
    Attributes:
        ...
        model_type (str): [LSTM, LGBM]
        
    Methods:
        create_lgbm_pipeline(self)
        create_lstm_pipeline(self)
    """
    def __init__(self, X, y, model_type='LSTM'):
        self.X = X
        self.y = y
        
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(self.X, self.y, test_size=0.2, stratify=self.X['GICS Sector'])
        self.model_type = model_type
        

    def create_lgbm_pipeline(self, features):
    
        misc_cols = [c for c in self.X.columns if c not in features]
